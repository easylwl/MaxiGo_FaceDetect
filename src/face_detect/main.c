...
*/    
    ...
    gpiohs_set_drive_mode(6, GPIO_DM_INPUT_PULL_DOWN);
 ...
 // char Receivebuffer[2];
    ...
    while (1)
    {
		...
		if (dirnow!=0)
		{
            ...
            uart_receive_data(UART_NUM, SoundDirStr, 10);
			strcpy(SoundDirStr,"D1\n");
            SoundDirStr[1]=(dirnow-1);
            rec_flag=0;
            uart_send_data(UART_NUM, SoundDirStr, 3);
            for (is=0;is<100;is++)
            {
                if (rec_flag==0)
                {
